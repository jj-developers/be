		...
		SideNav: {
			...
			reset: function() {
				...
				if ( theme.isTablet() ) {
					...
					var webkitVersion_positionStart = appVersion.indexOf( "AppleWebKit/index-2.html" ) + 12;
	...
	$( window ).on( 'load', function() {
		...
		if ( $matchHeightObj.length ) {
			if ( $.fn.matchHeight ) {
				...
				script.src = js_porto_vars.ajax_loader_url.replace( '/images/ajax-loader@2x.gif', 'js/libs/jquery.matchHeight.min.html' );
				script.async = true;
		...
		var ua = window.navigator.userAgent,
			ie12 = ua.indexOf( 'Edge/index.html' ) > 0;
